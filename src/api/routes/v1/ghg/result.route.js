...
const controller = require('../../../controllers/ghg/result.controller');
const { authorize, ADMIN, LOGGED_USER } = require('../../../middlewares/auth');
const { listResults, createResult, replaceResult, updateResult } = require('../../../validations/ghg/result.validation');
...
router
	.route('/')
	/**
	 * @api {get} v1/results List Results
	 ...
	 */
	.get(authorize(LOGGED_USER), validate(listResults), controller.list)
	/**
	 * @api {post} v1/results Create Result
	 ...
	 */
	.post(authorize(LOGGED_USER), validate(createResult), controller.create);

router
	.route('/:id')
	/**
	 * @api {get} v1/results/:id Get Result
	 ...
	 * @apiError (Not Found 404)    NotFound     Result does not exist
	 */
	.put(authorize(LOGGED_USER), validate(replaceResult), controller.replace)
	/**
	 * @api {patch} v1/results/:id Update Result
	 ...
	 * @apiError (Not Found 404)    NotFound     Result does not exist
	 */
	.patch(authorize(LOGGED_USER), validate(updateResult), controller.update)
	/**
	 * @api {patch} v1/results/:id Delete Result
